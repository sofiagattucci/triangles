...
public class TestBirthDayService {
  ...
  private EmployeeRepositoryImpl createEmployeeRepository() {
    ...
    when(empRep.findEmployeesBornOn(2, 29))
        .thenReturn(LocalDate.now().isLeapYear() ? (listOfEmployees.subList(2, 3)) : null);
    when(empRep.findEmployeesBornOn(2, 28))
        .thenReturn(
            LocalDate.now().isLeapYear()
                ? listOfEmployees.subList(3, 4)
                : listOfEmployees.subList(2, 4));
    return empRep;
  }
